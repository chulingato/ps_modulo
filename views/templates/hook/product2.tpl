...
{* <h3>{l s='prueba de modulo' mod='juanmodule'}</h3> *}
<div class="email_subscription block_newsletter" id="blockEmailSubscription_{$hookName}">
  ...
  <form action="{$urls.current_url}#blockEmailSubscription_{$hookName}" method="post">
    <input type="email" name="email" value="{$value}" placeholder="{l s='ingresa el ID del producto' d='Modules.Emailsubscription.Shop'}" required />
    {if $conditions}
      <p>{$conditions}</p>
    {/if}
    {hook h='displayNewsletterRegistration'}
    <input type="hidden" value="{$hookName}" name="blockHookName" />
    <input type="submit" value="buscar producto" name="submitNewsletter" />
